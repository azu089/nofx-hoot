...
export class SafetyService {
  ...
  private async checkL5(input: SafetyCheckInput): Promise<SafetyLayerResult> {
    ...
    // 3. 检查每日最大回撤（这里简化处理，实际应该计算真实回撤）
    // TODO: 实现真实的回撤计算（需要查询今日所有交易的盈亏）
    // 暂时跳过此检查，或者可以基于今日失败交易数作为简单估计

    return {
      passed: true,
      detail: `熔断检查通过: 失败=${failedCount}, 今日交易=${todayTradeCount}/${aiConfig.maxDailyTrades || '无限制'}`,
    };
  }
