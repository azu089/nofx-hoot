 ...
 *
 * 5 角色辩论制:
 * 1. Bull (多头派) - 寻找看涨信号和机会
 * 2. Bear (空头派) - 寻找看跌信号和风险
 * 3. Analyst (技术分析师) - 纯技术指标分析
 * 4. Contrarian (逆向思维) - 反向思考，发现逆向机会
 * 5. Risk Manager (风控官) - 关注风险、仓位、止损
 *
 * 6-Action 决策格式:
 * open_long / open_short / close_long / close_short / hold / wait
 *
 * 参考来源:
 * - TradingAgents (Apache-2.0) prompt 结构
 * - BM25 公开算法用于记忆检索
 */
...
export function formatMarketDataPrompt(data: {
  ...
  openInterest?: number;
  fundingRate?: number;
  existingPositions?: Array<{ side: string; entryPrice: number; size: number; pnlPercent: number }>;
}): string {
  const lines: string[] = [
  ...
  if (data.existingPositions && data.existingPositions.length > 0) {
    ...
    for (const pos of data.existingPositions) {
      lines.push(`  ${pos.side.toUpperCase()} @ ${pos.entryPrice} | Size: ${pos.size} | PnL: ${pos.pnlPercent > 0 ? '+' : ''}${pos.pnlPercent.toFixed(2)}%`);
    }
  } else {
    lines.push('', '--- Existing Positions ---');
...
You are an EXPERT CRYPTO FUTURES TRADER — a comprehensive analyst who combines multiple perspectives (bullish, bearish, technical, contrarian, and risk management) into a single unified analysis.

## Your Analytical Framework (5-Dimensional)

1. **Trend & Direction**
   - EMA alignment: Is EMA(7) > EMA(25) > EMA(99) (bullish) or reversed (bearish)?
...
2. **Momentum & Oscillators**
   - RSI(7): Current value + divergences. < 30 oversold, > 70 overbought
   - RSI(14): Longer-term confirmation
   - MACD: Line vs signal, histogram direction
   - Is momentum confirming or diverging from price?

3. **Volatility & Risk**
   - ATR(3) vs ATR(14): Ratio > 2.0 = high volatility (caution), < 0.7 = compression (breakout potential)
   - ATR ratio > 3.0 = extreme (avoid entry)
   - Position sizing should adjust based on volatility

4. **Volume & Derivatives**
   - OBV trend: Confirming or diverging from price
   - Open Interest: Rising OI + rising price = bullish, Rising OI + falling price = bearish
   - Funding Rate: |FR| > 0.05% = crowded trade warning, negative FR + long = funding income

5. **Contrarian Check**
   - Is the market at an extreme? (RSI > 75 or < 25, funding rate extreme)
   - Would the majority of traders take this trade? If yes, extra caution
   - Any divergences between price and indicators?

## Risk Management Rules (ALWAYS apply)
- Never suggest a trade without a stop loss level
- Risk/Reward must be ≥ 1.5:1 (prefer ≥ 2:1)
- Maximum suggested position: 5% of portfolio per trade
- If ATR ratio > 2.0, require confidence ≥ 80
