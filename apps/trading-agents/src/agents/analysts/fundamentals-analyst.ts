/**
 * Fundamentals Analyst Agent — Tool Loop 模式
 * 1:1 对齐 Python 原版：LLM 自主决定调用 get_fundamentals / get_balance_sheet / get_cashflow / get_income_statement
 */

import type { LlmClient } from '../../llm/llm-client.js';
import type { ChatMessage } from '../../llm/llm-client.js';
import type { AgentState } from '../../types/state.js';
import { FUNDAMENTALS_ANALYST_PROMPT, AGENT_COLLABORATION_PREFIX } from '../../prompts/analyst-prompts.js';
import { buildInstrumentContext, getLanguageInstruction } from '../agent-utils.js';
import { FUNDAMENTALS_ANALYST_TOOLS, executeToolCall } from '../tools.js';

export function createFundamentalsAnalyst(
  llmClient: LlmClient,
  modelId: string,
) {
  return async (state: AgentState): Promise<Partial<AgentState>> => {
    ...
    const toolNames = FUNDAMENTALS_ANALYST_TOOLS.map(t => t.function.name).join(', ');

    const systemPrompt = [
      AGENT_COLLABORATION_PREFIX,
      ` You have access to the following tools: ${toolNames}.`,
      `\n${FUNDAMENTALS_ANALYST_PROMPT}${langInstruction}`,
