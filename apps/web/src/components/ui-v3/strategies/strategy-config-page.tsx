...
export function StrategyConfigPage({
  ...
  const [blackSwanType, setBlackSwanType] = useState<'coin_drop' | 'account_loss'>('account_loss')
  ...
  // 风险模板变化时更新参数
  useEffect(() => {
    const preset = riskPresets[selectedRisk]
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] text-[#F8F8FC]">
      ...
      <div className="p-4 max-w-2xl mx-auto pb-36">
        {/* 主配置卡片 - 一体化设计 */}
        <div className="bg-[#12121A]/80 backdrop-blur-xl border border-[#1E1E2E] rounded-2xl overflow-hidden">
              ...
              <div className="flex bg-[#0A0A0F] rounded-lg p-0.5">
                {[
                  ...
                  <button
                    type="button"
                    key={template.id}
                    onClick={() => setSelectedRisk(template.id as typeof selectedRisk)}
                    className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
