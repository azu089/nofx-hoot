...
type FilterTab = 'all' | 'research' | 'solo' | 'debate';
...
function getStrategyCoins(s: AiStrategy): string[] {
  try {
    return (s.coinSourceConfig as any)?.coins || [];
  } catch {
    return [];
  }
}
...
function ResearchCard({
  ...
  const intervalMin = (session.cyclingConfig as any)?.intervalMinutes ?? 0;
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        {totalCycles > 0 && (
          <span className="px-2 py-0.5 bg-[#1E1E2E] text-[#06B6D4] text-xs rounded font-mono">
            #{totalCycles}/{(session.cyclingConfig as any)?.maxCycles || '\u221E'}
          </span>
        )}
      </div>
...
function StrategyCard({
  ...
  const badgeCfg = isGrid
    ? { Icon: LayoutList, label: t('modes.grid'), color: '#06B6D4', bg: 'bg-[#06B6D4]/15 text-[#06B6D4]', iconColor: 'text-[#06B6D4]' }
    : isDebate
...
export function UnifiedAiList() {
  ...
  const initialFilter: FilterTab = (['all', 'research', 'solo', 'debate'] as const).includes(initialFilterParam as FilterTab)
  ...
  // 数据请求
  const { data: researchData, isLoading: loadingResearch } = useResearchHistory(1, 20);
  ...
  const unifiedItems = useMemo<UnifiedAiItem[]>(() => {
    ...
    if (filter === 'solo') return items.filter((i) => i.type === 'strategy' && (i.data as AiStrategyWithPnl).tradingMode === 'solo');
    if (filter === 'debate') return items.filter((i) => i.type === 'strategy' && (i.data as AiStrategyWithPnl).tradingMode !== 'solo');
    return items;
  ...
  const activeCount = useMemo(() => {
    let count = 0;
    if (researchData?.data) {
      count += researchData.data.filter(
        (s) => s.status === 'running' || s.campaignStatus === 'running',
      ).length;
    }
    if (strategyData?.data) {
      count += strategyData.data.filter((s) => s.isActive).length;
    }
    return count;
  }, [researchData, strategyData]);

  const totalPnl = useMemo(() => {
    if (!strategyData?.data) return 0;
    return strategyData.data.reduce((sum, s) => sum + Number(s.totalPnl), 0);
  }, [strategyData]);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] text-[#F8F8FC]">
      ...
      <div className="px-4 py-3">
        <div className="glass-card-hd grid grid-cols-3">
