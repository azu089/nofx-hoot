...
export function AIStrategyDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] text-[#F8F8FC] w-full">
      {/* 顶部导航栏 */}
      <header className="sticky top-0 z-30 bg-[#0A0A0F] pt-[env(safe-area-inset-top)] [transform:translateZ(0)] border-b border-[#1E1E2E]">
                ...
                <div className="glass-border-glow glass-card p-4">
                  ...
                  <div>
                    ...
                    {(riskControlConfig?.btcEthMaxPositionValueRatio || riskControlConfig?.altcoinMaxPositionValueRatio) && (
                      <ConfigRow label={t('detail.positionValueRatio')} value={`BTC/ETH ${riskControlConfig?.btcEthMaxPositionValueRatio ?? 5}x / ${t('detail.altcoin')} ${riskControlConfig?.altcoinMaxPositionValueRatio ?? 1}x`} />
                    )}
                      <ConfigRow label={t('detail.executionCycle')} value={`${strategy.intervalMinutes} ${t('common.min')}`} />
                      {/* 止停条件（有值时显示） */}
                      {!!(strategy.stopConditions?.maxCycles || strategy.stopConditions?.profitTargetPercent || strategy.stopConditions?.maxLossPercent) && (
