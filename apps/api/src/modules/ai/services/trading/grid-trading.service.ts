...
export class GridTradingService {
  ...
  private async placeGridLimitOrder(
    ...
    // neutral 模式方向纠正：低于市价的层应挂买单，高于市价的层应挂卖单
    // 防止 AI 在高价层下买单（PostOnly 会拒绝，但浪费 API 调用和决策机会）
    if (level && level.state === 'empty' && (state.currentDirection ?? 'neutral') === 'neutral') {
      ...
      if (currentPrice > 0) {
        ...
        if (side !== correctSide) {
          this.logger.warn(`[网格] 方向纠正: L${levelIndex + 1} 价格${level.price.toFixed(2)} ${level.price <= currentPrice ? '<' : '>'} 市价${currentPrice.toFixed(2)}，${side}→${correctSide}`);
  ...
  private buildDisplayFromExchange(
    ...
    const display: any[] = state.gridLines.map((l, i) => ({
      lv: i + 1,
      p: +l.price.toFixed(4),
      s: currentPrice > 0 ? (l.price <= currentPrice ? 'buy' : 'sell') : l.side, // 对齐 nofx：empty 层 side 按当前价动态计算
      st: 'empty',
    }));
