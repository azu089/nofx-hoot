      ...
      if (depthCfg.investDebateRounds > 0) {
        const stage2 = await this.runStage(2, '投资辩论', async () => {
          ...
          const debateConfig: DebateConfig = {
            ...
            maxRounds: depthCfg.investDebateRounds + 2, // +2 因为 debate.service 用 maxRounds 作为总轮数
            memoryPrompt,
          };
...
Additional fields required in your JSON:
- "leverage": recommended leverage (1-20)
- "positionSizePercent": position size as % of portfolio (1-10)`;
          ...
          try {
            const execResult = await this.execution.executeDecision(
              ...
              config.apiKeyId,
              {
                ...
                confidence: decision.confidence,
                leverage: decision.leverage,
                positionSizeUSD: decision.positionSizePercent, // 由 execution 层转换
